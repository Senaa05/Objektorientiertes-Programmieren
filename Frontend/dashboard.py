...
def zeige_dashboard():
    ...
    def zeige_popup_ueberfaellig():
        ...
        with ui.dialog() as dialog, ui.card().classes("max-w-lg w-full"):
            ui.label("⚠️ Überfällige Bücher").classes("text-lg font-bold")
            ui.label(
                "Mindestens eine deiner Ausleihen ist überfällig. Bitte gib das Buch zurück oder melde dich beim Admin."
            ).classes("text-gray-600 mb-1")

            for eintrag in eintraege:
                titel = eintrag.get("titel", "Unbekanntes Buch")
                ui.label(f"• {titel} (fällig seit {eintrag['faelligkeit']})").classes("text-red-700")
    ...
    def zeige_reminder_bald_faellig():
        ...
        with ui.dialog() as dialog, ui.card().classes("max-w-lg w-full"):
            ui.label("⏰ Erinnerung: Bald fällige Ausleihen").classes("text-lg font-bold")
            ui.label(
            ...
            for eintrag in eintraege:
                titel = eintrag.get("titel", "Unbekanntes Buch")
                ui.label(f"• {titel} (fällig am {eintrag['faelligkeit']})").classes("text-amber-700")
    ...
    # ── Navigationsleiste ──
    with ui.header().classes("bg-blue-950 text-white px-6 py-3"):
        with ui.row().classes("items-center justify-between w-full"):
            ui.label("📚 Bibflow").classes("text-xl font-bold")
